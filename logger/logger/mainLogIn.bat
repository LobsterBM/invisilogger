...
echo(%token%|findstr /r "[^0-9a-zA-Z]" >nul && (
  ...
  rem add to log file 
  echo %dateformat% TokenError : non alphanumeric symbol found during login >> log.txt
  goto exitloop
)
...
login.bat|findstr /C:"UserConnectionSuccess" >nul 2>&1
if not errorlevel 1 (
...
echo timeChecker/restoreLocalLogIn/%token%/%dateformat% >> connection.txt
echo %dateformat% ValidationError : Could not validate connection during login >> log.txt
goto exitloop
)
goto loop
:exitloop
